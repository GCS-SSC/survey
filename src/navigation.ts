...
import { validateAnswers, type SurveyAnswers, type SurveyError } from './answers.js'
...
export const useSurveyFlow = (options: {
  ...
  const pageIndex = computed(() =>
    Math.max(
      0,
      route.value.pages.findIndex((page) => page.id === currentId.value)
    )
  )
  const page = computed(() => route.value.pages[pageIndex.value])
  watch(
    () => toValue(options.definition),
    () => {
  ...
  watch(
    () => route.value.pages.map((item) => item.id),
    (ids, previous) => {
  ...
  const setAnswer = (id: string, value: string) => {
    if (!page.value?.activeQuestionIds.includes(id)) return
    options.onChange(
      pruneAnswers(toValue(options.definition), { ...toValue(options.answers), [id]: value })
    )
    complete.value = false
    errors.value = {}
  }
  ...
  const next = () => {
    ...
    errors.value = Object.fromEntries(
      Object.entries(allErrors).filter(([id]) => page.value?.activeQuestionIds.includes(id))
    )
    ...
    if (pageIndex.value + 1 < route.value.pages.length)
      ...
      if (Object.keys(allErrors).length) {
        const invalidPage = route.value.pages.find((item) =>
          item.activeQuestionIds.some((id) => Object.hasOwn(allErrors, id))
        )
        if (invalidPage) currentId.value = invalidPage.id
        return false
      }
      complete.value = true
    }
  ...
  const back = () => {
    if (complete.value) complete.value = false
    else if (pageIndex.value > 0) currentId.value = route.value.pages[pageIndex.value - 1]!.id
    errors.value = {}
  }
  return {
    route,
