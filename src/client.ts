import { z } from 'zod'
import { surveySchema, type SurveyDefinition } from './index.js'
...
export const pushSurvey = async (options: {
  ...
  if (!response.ok) throw new Error(`Survey import failed (${response.status})`)
  const result = z
    .object({
      survey: z.object({
        id: z.uuid(),
  ...
  if (
    result.survey.agencyId !== options.agencyId ||
