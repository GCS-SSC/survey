import { z } from 'zod'
import { identifier, type SurveyDefinition } from './model.js'
...
export const validateAnswers = (
  ...
  const errors: Record<string, SurveyError> = Object.create(null)
  const ids = new Set(definition.questions.map((question) => question.id))
  for (const key of Object.keys(answers)) if (!ids.has(key)) errors[key] = 'unknown'
  ...
  for (const question of definition.questions) {
    ...
    if (!value.trim()) {
      if (mode === 'submit' && question.required) errors[question.id] = 'required'
      continue
    }
    ...
    if (question.type === 'date' && !isCalendarDate(value)) errors[question.id] = 'date'
