...
export const matchesCondition = (
  ...
  const results = condition.conditions.map((predicate) => {
    if (!activeIds.has(predicate.questionId)) return false
    const value = Object.hasOwn(answers, predicate.questionId) ? answers[predicate.questionId]! : ''
    if (predicate.operator === 'answered') return Boolean(value.trim())
    if (predicate.operator === 'notAnswered') return !value.trim()
...
export const resolveSurvey = (definition: SurveyDefinition, answers: SurveyAnswers) => {
  ...
  const active = new Set<string>(),
    pages: ResolvedPage[] = []
  const visible = (when?: SurveyCondition) => !when || matchesCondition(when, answers, active)
  const include = (ids: string[]) =>
    ids.filter((id) => {
      const question = questions.get(id)
      if (!question || !visible(question.visibleWhen)) return false
      active.add(id)
      return true
    })
  let index = 0
  while (index < structured.pages.length) {
    ...
    for (const section of page.sections) {
      if (!visible(section.visibleWhen)) continue
      const questionIds = include(section.questionIds)
      const subsections: ResolvedSubsection[] = []
    ...
    const target =
      page.branches.find((branch) => matchesCondition(branch.when, answers, active))?.destination ??
      page.next
    if (target?.kind === 'end') break
    const nextIndex =
      target?.kind === 'page'
        ? structured.pages.findIndex((candidate) => candidate.id === target.pageId)
        : index + 1
    if (nextIndex <= index) throw new Error('Invalid forward survey route')
    index = nextIndex
  }
  const retained: SurveyAnswers = Object.create(null)
  for (const id of active) if (Object.hasOwn(answers, id)) retained[id] = answers[id]!
...
export const pruneAnswers = (definition: SurveyDefinition, answers: SurveyAnswers): SurveyAnswers =>
  resolveSurvey(definition, answers).answers
